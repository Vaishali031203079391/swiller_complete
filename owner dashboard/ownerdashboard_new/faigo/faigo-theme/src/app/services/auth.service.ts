...
import { AuthData } from '../shared/models/auth.model';
...
export class AuthService {
  ...
  constructor(private store: LocalStoreService, private http: HttpClient, private router: Router, private activatedRoute: ActivatedRoute) {
    this.checkAuth();
  }
  ...
  createUser(name: string, email: string, confirmpassword: string) {
    const authData: AuthData = {
      name: name,
      email: email,
      confirmpassword: confirmpassword
    }
    this.http.post<{ resturant: any, token: string, expiresIn: number }>(BACKEND_URL + "register", authData).subscribe(response => {
      const token = response.token;
      this.user = response.resturant;
      ...
      if (token) {
        ...
        this.router.navigate(['/forms/settings']);
      }
    }, error => {
      console.log(error);
  ...
  login(email: string, confirmpassword: string) {
    this.router.navigate(['/forms/wizard']);
    // const authData = {
    //   email: email,
    //   confirmpassword: confirmpassword
    // }
    // this.http.post<{ resturant: any, token: string, expiresIn: number }>(BACKEND_URL + "login", authData).subscribe(response => {
    ...
    //     this.saveAuthData(token, expirationDate, response.resturant);
